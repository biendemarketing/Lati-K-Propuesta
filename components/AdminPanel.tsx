...
-- Run this in your Supabase SQL Editor to enable status and comments.

-- 1. Add a status column to the 'proposals' table
-- Note: This is now handled by adding 'status' to the ProposalData JSONB object. No ALTER TABLE needed if you re-save a proposal.
-- If you want a dedicated column (better for filtering), run this:
-- ALTER TABLE proposals ADD COLUMN status TEXT DEFAULT 'Draft';

-- 2. Create the table to store comments
...
-- 4. Create a policy that allows anyone to insert
CREATE POLICY "Allow public insert" ON proposal_comments FOR INSERT WITH CHECK (true);

-- 5. Create a policy that allows authenticated users (admins) to read
CREATE POLICY "Allow admin read" ON proposal_comments FOR SELECT USING (auth.role() = 'authenticated');
*/
